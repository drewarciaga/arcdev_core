...
class Organizer extends MyBaseModel
{
    ...
    public $rules = [
        ...
        'middle_name' => 'max:200',
        'remarks' => 'max:2000',
        'waiver' => 'nullable|string|max:10000',
    ...
    {
        if ( is_null($this->organizer_type) ) {
            return '';
        }

        if ( $this->organizer_type instanceof OrganizerType ) {
            return $this->organizer_type->label();
        }

        $organizerType = OrganizerType::tryFrom((int) $this->organizer_type);

        return !empty($organizerType) ? $organizerType->label() : '';
    }
}